    ...
    App::new()
        .add_plugins(DefaultPlugins.set(WindowPlugin {
            primary_window: Some(Window {
                resolution: WindowResolution::new(DEFAULT_SCREEN_RESOLUTION.0, DEFAULT_SCREEN_RESOLUTION.1),
                title: "".to_string(),
                ..default()
            }),
            ..default()
...
enum Algorithm {
    Insertion, Selection, Merge, Schrödinger, Validation
}
...
fn settings_widget(mut contexts: EguiContexts, mut selected: ResMut<SelectedAlgorithm>, mut max_speed: ResMut<MaxSpeed>, mut speed_limit: ResMut<SpeedMode>,
    mut operations: ResMut<Operations>, mut v: ResMut<List>, mut n: ResMut<NumberOfItems>, mut delay: ResMut<Delay>, mut sorting: ResMut<SortingOngoing>, 
    mut observed: ResMut<Observed>, mut insertion_step: ResMut<InsertionStep>, mut selection_step: ResMut<SelectionStep>, mut merge_step: ResMut<MergeStep>, 
    mut paused: ResMut<Paused>, mut sort: ResMut<Sort>, mut sound: ResMut<Sound>) {
    egui::Window::new("Controller").show(contexts.ctx_mut(), |ui| {
        ...
        if selected.0 != Validation {
            ...
            if max_speed.0 == Speed::Max {
                ...
                if speed_limit.0 == SpeedLimit::Low {
                    ...
                    delay.0 = 0;
                    ui.add(egui::Slider::new(&mut operations.0, 0..=100000).text("Operations/Frame"));
                }
            } 
        }
        else {
        ...
        if sorting.0 && !observed.0 && selected.0 == Schrödinger {
            ...
            ui.label(format!("Until the list is observed, it is both sorted and unsorted"));
...
fn speed_controller(mut operations: ResMut<Operations>, delay: Res<Delay>, sorting: ResMut<SortingOngoing>, mut sort: ResMut<Sort>, paused: Res<Paused>, max_speed: Res<MaxSpeed>, selected: Res<SelectedAlgorithm>) {
...
fn insertion_sort(mut sort: ResMut<Sort>, mut step: ResMut<InsertionStep>, mut v: ResMut<List>,
    mut selected: ResMut<SelectedAlgorithm>, operations: Res<Operations>, mut end_step: ResMut<AnimationStep>,
    mut pitch_assets: ResMut<Assets<Pitch>>, mut frequency: ResMut<PitchFrequency>, mut commands: Commands, sound: Res<Sound> //comment out to run insertion test
) {
    if sort.0 && selected.0 == Algorithm::Insertion {
        if operations.0 != 0 && sound.0 {                                                                          //comment out to run insertion test
            frequency.0 = 200. + 1500.0 * step.0.1.pow(3) as f32 / v.0.len().pow(3) as f32;               //comment out to run insertion test
            commands.spawn(PitchBundle {                                                                    //comment out to run insertion test
...
fn selection_sort(mut sort: ResMut<Sort>, mut step: ResMut<SelectionStep>, mut v: ResMut<List>,
    mut selected: ResMut<SelectedAlgorithm>, operations: Res<Operations>,
    mut pitch_assets: ResMut<Assets<Pitch>>, mut end_step: ResMut<AnimationStep>,
    mut frequency: ResMut<PitchFrequency>, mut commands: Commands, sound: Res<Sound>,                               //comment out to run selection test
    ) {
    if sort.0 && selected.0 == Algorithm::Selection {                                                               //comment out to run selection test
        if operations.0 != 0 && sound.0 {                                                                           //comment out to run selection test
            frequency.0 = 200. + 1500.0 * step.0.1.pow(3) as f32 / v.0.len().pow(3) as f32;                //comment out to run selection test
            commands.spawn(PitchBundle {                                                                    //comment out to run selection test
...
fn merge_sort(mut sort: ResMut<Sort>, mut step: ResMut<MergeStep>, mut v: ResMut<List>,
     mut selected: ResMut<SelectedAlgorithm>, operations: Res<Operations>, mut end_step: ResMut<AnimationStep>,
     mut pitch_assets: ResMut<Assets<Pitch>>, mut frequency: ResMut<PitchFrequency>, mut commands: Commands, sound: Res<Sound>   //comment out to run merge test
    ) {
    if sort.0 && selected.0 == Algorithm::Merge {                                                                                //comment out to run merge test
        ...
        if operations.0 != 0 && sound.0{                                                                                            //comment out to run merge test
            frequency.0 = 200. + 1500.0 * i.pow(3) as f32 / v.0.len().pow(3) as f32;                                     //comment out to run merge test
            commands.spawn(PitchBundle {                                                                                    //comment out to run merge test
        ...
        while operation < operations.0 {
            ...
            if step.0.0 >= v.len() {
                ...
                if i1 < v1.len() {
                    ...
                    step.0.4 = v[step.0.0*2*step.0.1..min(step.0.0*2*step.0.1+step.0.0, v.len())].to_vec();
                    if step.0.0*2*step.0.1+step.0.0 < v.len() {
                        step.0.5 = v[step.0.0*2*step.0.1+step.0.0..min(step.0.0*2*step.0.1+step.0.0*2, v.len())].to_vec();
                    }
                    else {
                        step.0.5 = vec![];
                    }
...
fn schrödinger_sort(mut sort: ResMut<Sort>, observed: Res<Observed>, mut v: ResMut<List>,
    mut selected: ResMut<SelectedAlgorithm>, operations: Res<Operations>, mut end_step: ResMut<AnimationStep>,
    mut pitch_assets: ResMut<Assets<Pitch>>, mut frequency: ResMut<PitchFrequency>, mut commands: Commands, sound: Res<Sound>
) {
...
fn end_animation(v: Res<List>, mut step: ResMut<AnimationStep>, mut operations: ResMut<Operations>, mut selected: ResMut<SelectedAlgorithm>,
     mut sort: ResMut<Sort>, mut sorting: ResMut<SortingOngoing>,mut pitch_assets: ResMut<Assets<Pitch>>,
     mut frequency: ResMut<PitchFrequency>, mut commands: Commands, sound: Res<Sound>) {
...
fn render_list(mut commands: Commands, v: ResMut<List>, sprites: Query<Entity, With<Sprite>>, windows: Query<&Window>, 
    selected: Res<SelectedAlgorithm>, insertion_step: Res<InsertionStep>, selection_step: Res<SelectionStep>, 
    merge_step: Res<MergeStep>, end_step: Res<AnimationStep>, ongoing: Res<SortingOngoing>, observed: Res<Observed>) {
    ...
    for (n, i) in v.0.iter().zip(0..) {
        ...
        commands.spawn(SpriteBundle {
            sprite: Sprite {
                color: color,
                custom_size: Some(Vec2::new( 0.9*window_width/len, (window_height-200.)* *n as f32/len)),
                anchor: Anchor::BottomLeft,
                ..default()
            },
            transform: Transform::from_translation(Vec3::new(-window_width/2. + i as f32 * window_width/len + 0.05*window_width/len, -window_height/2., 0.)),
            ..default()
        });
    }
...
#[test]
fn insertion_sort_test() { //checking if insertion sort is correct for one random vec for each len 1-1000
...
#[test]
fn selection_sort_test() { //checking if selection sort is correct for one random vec for each len 1-1000
    use rand::{Rng, thread_rng};
    let mut app = App::new();
...
#[test]
fn merge_sort_test() { //checking if merge sort is correct for one random vec for each len 1-1000
    use rand::{Rng, thread_rng};
    let mut app = App::new();
